...
class Solution:
    ...
    # Time Complexity: O(4⋅S⋅(log⁡2max⁡(M,N)−log⁡4S))
    # The log2Max(M, N) represents the maximum number of levels in the recursion tree. However, not all of those levels will have 4 * S calls. The first level  

    # That said, simply stating the complexity as S·log2Max(M, N) should be sufficient. You don't necessarily need to include the -log4S part, it just provides a       tighter upper bound.
    # Space Complexity: log⁡2max⁡(M,N)